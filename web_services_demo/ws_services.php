...
if ($actions == 'loginfos') {
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        if (validate_password($obj->password, $good_hash)) {
            ...
            if ($obj->applicationid == '0') {
                ...
                //$filterstr = "select *  from application WHERE is_assignto_fos='1' and am_accaptance='1' and fosid = '" . $row['agencymanagerid'] . "' and fos_completed='0' and fos_completed_status!=20 and runsheet='0' " . $where . " ORDER BY applicationid ASC";
                $filterstr = "select *  from application WHERE is_assignto_fos='1' and am_accaptance='1' and fosid = '" . $row['agencymanagerid'] . "' and fos_completed='0' and fos_completed_status!=20 and runsheet='0' " . $where . " and (PTP_Date is null or ptp_datetime ='') ORDER BY applicationid ASC";
                //and (PTP_Date is null or ptp_datetime ='')
            } else {
                $filterstr = "select *,(select fosstatusdrropdown.status from fosstatusdrropdown WHERE fosstatusdrropdown.fosstatusdrropdownid=application.fos_completed_status)as status  from application WHERE  applicationid='" . $obj->applicationid . "' and  is_assignto_fos='1' and am_accaptance='1' and fosid = '" . $row['agencymanagerid'] . "' " . $where . " ORDER BY STR_TO_DATE(PTP_Date,'%d-%m-%Y') ASC  ";
            }
            // echo $filterstr;
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        if (validate_password($obj->password, $good_hash)) {
            ...
            $totalapp = mysqli_query($dbconn, "select count(*) as totalassignapp from application WHERE is_assignto_fos='1' and fosid = '" . $row['agencymanagerid'] . "' and fos_completed='0' and fos_completed_status='0' " . $where . " ");
            ...
            $totalcompapp = mysqli_query($dbconn, "select count(*) as completedapp from application WHERE is_assignto_fos='1' and fosid = '" . $row['agencymanagerid'] . "' and fos_completed > '0'  and fos_completed_status NOT IN (3,5,17) " . $where . " ");
            $totalcompletedcount = mysqli_fetch_assoc($totalcompapp);
            $countcompletedapp = $totalcompletedcount['completedapp'];

            $total = mysqli_query($dbconn, "select sum(totalamt) as totalPayment_Collected_Amount from application WHERE is_assignto_fos='1' and fosid = '" . $row['agencymanagerid'] . "' and fos_completed > '0' " . $where . " ");
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        if (validate_password($obj->password, $good_hash)) {
            ...
            from application where application.fosid = '" . $row['agencymanagerid'] . "' " . $where . "
            ...
            and application.fos_completed_status=0 " . $where . "
            UNION ALL
            select 20 as 'fosstatusdrropdownid','Rejected Count',count(*) as StatusCount from application where  application.fosid = '2003'
            and application.fos_completed_status=20 " . $where . "";
            //echo $totalapp;
            //exit;
            /*
            select 17,'Broken PTP',count(*)
            from application where application.fosid = '" . $row['agencymanagerid'] . "' and fos_completed_status=17 " . $where . "            
            select 17,'Broken PTP',count(*)
            from application where application.fosid = '" . $row['agencymanagerid'] . "' " . $where . "
            and  ((STR_TO_DATE(application.PTP_date,'%d-%m-%Y %T') < '" . date('Y-m-d') . "'
            and application.PTP_date != '' and fos_completed_status=17) or (STR_TO_DATE(application.ptp_datetime,'%d-%m-%Y %T') < '" . date('Y-m-d') . "'
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        if (validate_password($obj->password, $good_hash)) {
            ...
            if ($obj->fosstatusdrropdownid == '5') {
                ...
                //$filterstr = "select *  from application WHERE  runsheet=0 and   is_assignto_fos='1' and am_accaptance='1' and fosid = '" . $row['agencymanagerid'] . "'  ";
                //and  ((STR_TO_DATE(PTP_Date,'%d-%m-%Y %H:%i:%s') < '" . date('Y-m-d') . "' and PTP_Date!='' and fos_completed_status=17) or (STR_TO_DATE(ptp_datetime,'%d-%m-%Y %H:%i:%s') < '" . date('Y-m-d') . "' and ptp_datetime!='' and fos_completed_status=17))
            } else {
...
} else if ($actions == 'bankmasterlist') {
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        if (validate_password($obj->password, $good_hash)) {
            $filterstr = "select iBankId,strBankName from bankmaster where isDelete =0 and iStatus =1 ORDER BY strBankName ASC";
            $result1 = mysqli_query($dbconn, $filterstr);
            if (mysqli_num_rows($result1) > 0) {
                $jCounter = 1;
                $data[] = array(
                    "iBankId" => "",
                    "strBankName" => "Select Bank"
                );
                while ($row = mysqli_fetch_assoc($result1)) {
                    $data[] = array(
                        "iBankId" => $row['iBankId'],
                        "strBankName" => $row['strBankName']
                    );
                    $jCounter++;
                }
                $output['bankmasterlist'] = $data;
                $output['message'] = 'Data Found';
                $output['success'] = '1';
            } else {
                $output['message'] = 'Not Data Found';
