                    ...
                    if ($ValCounter == 0) {
                        ...
                        for ($icounter = 0; $icounter < count($slice); $icounter++) {
                            ...
                            if ($icounter == $LANNUMBERColumnCounter) {
                                ...
                                if (trim($LANNUMBER) != "") {
									$lanapp = mysqli_query($dbconn, "SELECT * FROM `application`  where  Account_No='" . $LANNUMBER . "' AND am_accaptance in ('1','0')");
                                    if (mysqli_num_rows($lanapp) == 0) {
                                        $errorString .= "Row " . $ValCounter . " & Account No =" . $LANNUMBER . " not match <br/>";
                                    } else {
                                        $rowlanapp = mysqli_fetch_array($lanapp);
                                        $LANNUMBERInRow = $rowlanapp['Account_No'];
                                    }
                                }
                            }
							...
							if ($icounter == $LANNUMBERColumnCounter) {
                                $LANNUMBER = $slice[$icounter];
                                if (trim($LANNUMBER) != "") {
									$lanappbkt1 = mysqli_query($dbconn, "SELECT * FROM `application`  where  Account_No='" . $LANNUMBER . "' AND am_accaptance in ('1','0') ");
                                    if (mysqli_num_rows($lanappbkt1) > 0) {
                                        ...
                                        //     $errorString .= "Restricted for Account No" . $LANNUMBER . " &row No =" . $ValCounter . " <br/>";
                                        // } else 
										if(in_array($rowlanappbkt1['fos_completed_status'], $fos_completed_status)) {
											...
											$errorString .= "Restricted for Account No " . $LANNUMBER . " & Status =" . $completedstatus . " <br/>";
										} else {
                                            $LANNUMBERInRow = $rowlanappbkt1['Account_No'];
                            ...
                            if ($icounter == $AgencyColumnCounter) {
								...
								if (trim($Agencyname) != "") {
									$agencyname = mysqli_query($dbconn, "SELECT * FROM `agencymanager`  where  loginId like '" . $Agencyname . "' AND type='FOS' and isDelete=0 ");
														
                                    if (mysqli_num_rows($agencyname) == 0) {
                                    	
										$errorString .= "Row " . $ValCounter . " & Restricted for Account No " . $LANNUMBER . " & Agencyname =" . $agency . " not match <br/>";
                                    } else {
										...
										if($agencyid != 0){
											if($agencyid == $rowagencyname['agencyname']){
												...
												$errorString .= "Row " . $ValCounter . " & Restricted for Account No " . $LANNUMBER . " & Fos Agencyname =" . $agency . " not match <br/>";
											}
										}
                                    }
                                }
						...
						if ($key > 0) {
							...
							for ($icounter = 0; $icounter < count($slice); $icounter++) {
								if ($icounter == $LANNUMBERColumnCounter) {
									...
									$lanapp = mysqli_fetch_array(mysqli_query($dbconn, "SELECT * FROM `application`  where  Account_No='" . $LANNUMBERInRow . "' AND am_accaptance in ('1','0')"));
									$fos_completed_status = $lanapp['fos_completed_status'];
									$LANNUMBERInRow = $lanapp['Account_No'];                                    
							...
							if($fos_completed_status == 5){
								$insertString .= "fosid" . "=" . $fosid . ",agency = " . "'" . $agencyname . "' " . ", agencyid = " . $agencyid . $Whwere = " where  Account_No = " . "'" . $appid . "' and  fos_completed_status NOT IN ('1','10','11')  AND am_accaptance in ('1','0') ";
								...
								$insertString .= "is_assignto_fos" . "=" . "1" . ",ptp_datetime" . "=" . "''" .",PTP_Date" . "=" . "''" .",fos_submit_datetime" . "=" . "''". ",fos_completed_status" . "=" . "0" . ",fos_completed" . "=" . "0" . ",fosid" . "=" . $fosid . ",agency = " . "'" . $agencyname . "' " . ", agencyid = " . $agencyid . $Whwere = " where  Account_No = " . "'" . $appid . "' and  fos_completed_status NOT IN ('1','10','11')  AND am_accaptance in ('1','0')";
								//  and month(STR_TO_DATE(strEntryDate,'%d-%m-%Y %H:%i:%s'))= Month(STR_TO_DATE('" . $currentmonth . "','%d-%m-%Y')) and Year(STR_TO_DATE(strEntryDate,'%d-%m-%Y %H:%i:%s'))= Year(STR_TO_DATE('" . $currentmonth . "','%d-%m-%Y'))  
							}
//				 			echo $insertString;
				 			//exit;
