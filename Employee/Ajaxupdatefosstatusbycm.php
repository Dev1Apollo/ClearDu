...
if ($_POST['action'] == 'ListUser') {
    ...
    if ($_REQUEST['Account_No'] != "") {
        $where.=" and  Account_No = '" . $_REQUEST['Account_No'] . "' ";
    }
    ...
    $filterstr = "SELECT * FROM `application` " . $where . " and  is_assignto_am ='1' and am_accaptance='1'  ";
    $countstr = "SELECT count(*) as TotalRow FROM  `application` " . $where . " and   is_assignto_am ='1' and am_accaptance='1' ";
        ...
        <table class="table table-striped table-bordered table-hover dt-responsive" width="100%" id="tableC">
            ...
            <thead class="tbg">
                <tr>
                    ...
                    <th class="all">Emi/Collectible Amount</th>
                    <th class="all">Total POS Amount</th>
                    ...
                    <th class="all">Residence Address</th>
                    <th class="all">Business Address</th>
            ...
            <tbody>
                ...
                while ($rowfilter = mysqli_fetch_array($resultfilter)) {
                    ...
                    <tr >
                        ...
                        <td>
                            <div class="form-group form-md-line-input "><?php echo $rowfilter['Bank_Name']; ?> 
                            </div>
                        </td> 
                        <td>
                        ...
                        <td>
                            <div class="form-group form-md-line-input "><?php echo $rowfilter['cycle']; ?> 
                            </div>
                        </td>
                        <!-- <td>
                        ...
                        <td>
                            <div class="form-group form-md-line-input "><?php echo $rowfilter['Total_Pos_Amount']; ?> 
                            </div>
                        </td>
                        <!-- <td>
                        ...
                        <td>
                            <div class="form-group form-md-line-input "><?php echo $rowfilter['Business_Address']; ?> 
                            </div>
                        </td>
                        <td>
                        ...
                        <td>                             
                            <div class="form-group form-md-line-input "><?php echo $rowfilter['Area_Name']; ?> 
                            </div>
                        </td>
                        <?php
                        ...
                        <td>
                            <button type="button" class="btn blue btn-sm" onclick="return showDetail('<?php echo $rowfilter['applicationid']; ?>','<?php echo $rowfilter['Bank_Name'] ?>','<?php echo $rowfilter['Emi_amount']; ?>','<?php echo $rowfilter['Total_Pos_Amount']; ?>','<?php echo $rowfilter['PRODUCT']; ?>');"  ><i class="fa fa-history"></i></button>
...
if ($_POST['action'] == 'changefosstatus') {
    
    $row = mysqli_fetch_assoc(mysqli_query($dbconn,"SELECT * FROM `application` where applicationid='".$_POST['iApplicationid']."' and  is_assignto_am ='1' and am_accaptance='1'"));
    $fos_submit_date = date('d-m-Y');
    if ($_POST['Payment_Collected_Amount'] == "") {
        $Payment_Collected_Amount = '0';
    } else {
        $Payment_Collected_Amount = $_POST['Payment_Collected_Amount'];
    }
    if ($_POST['ptp_datetime'] == "") {
        $ptp_date = 0;
    } else {
        $ptp_date = $_POST['ptp_datetime'];
    }
    
    
    
    if ($_POST['fos_completed'] == '1') {
        $Payment_Collected_Date = date('d-m-Y H:i:s');
        $fos_completed = '1';
        $user_Data = array(
            "fosid" => $row['fosid'],
            "applicationid" => $_POST['iApplicationid'],
            "status" => $_POST['fos_completed'],
    ...
    } else if ($_POST['fos_completed'] == '2') {
        $Payment_Collected_Date = "NA";
        $fos_completed = '1';
    } else if ($_POST['fos_completed'] == '3') {
        $Payment_Collected_Date = "NA";
        $fos_submit_date = explode('-', $fos_submit_date);
        $ptp_date = explode('-', $_POST['ptp_datetime']);
        ...
        if($days==0){
            $_POST['fos_completed']=5;  
        }
    } else if ($_POST['fos_completed'] == '4') {
        $Payment_Collected_Date = "";
        $fos_completed = '1';
    } else if ($_POST['fos_completed'] == '6') {
        $Payment_Collected_Date = "NA";
        $fos_completed = '1';
    } else if ($_POST['fos_completed'] == '7') {
        $Payment_Collected_Date = "NA";
        $fos_completed = '1';
    } else if ($_POST['fos_completed'] == '16') {
        $Payment_Collected_Date = "NA";
        $fos_completed = '1';
    } else if ($_POST['fos_completed'] == '10') {
        $Payment_Collected_Date = date('d-m-Y H:i:s');
        $fos_completed = '1';
    } else if ($_POST['fos_completed'] == '11') {
        $Payment_Collected_Date = date('d-m-Y H:i:s');
        $fos_completed = '1';
    } if ($_POST['fos_completed'] == '18') {
        $Payment_Collected_Date = date('d-m-Y H:i:s');
        $fos_completed = '1';
        $user_Data = array(
            "fosid" => $row['fosid'],
        ...
        $insert = $connect->insertrecord($dbconn, 'applicationpaymentcollectedhistory', $user_Data);
        // $where = " where applicationid = '" . $_POST['iApplicationid'] . "' ";
        $sqlQry = mysqli_fetch_assoc(mysqli_query($dbconn,"select * from application ".$where.""));
        $_POST['totalamt'] = $_POST['totalamt'] + $sqlQry['totalamt'];
    } 


    $applicationdetails = array(
        "fos_completed_status" => $_POST['fos_completed'],
        "fos_completed" => $fos_completed,
        "fos_comment" => $_POST['strComment'],
        "AlternetMobileNo" => $_POST['AlternetMobileNo'],
        'ptp_datetime' => $_POST['ptp_datetime'],
        "fos_submit_datetime" => date('d-m-Y H:i:s'),
        "Payment_Collected_Amount" => $Payment_Collected_Amount ? $Payment_Collected_Amount : 0,
        "penal" => $_POST['penal'] ? $_POST['penal'] : 0,
        "runsheet" => '0',
        "totalamt" => $_POST['totalamt'] ? $_POST['totalamt'] : 0,
    ...
    $rowAgancy = mysqli_fetch_assoc(mysqli_query($dbconn,"SELECT * FROM `agencymanager` where agencymanagerid='".$row['fosid']."'"));
    $userData = array(
        "fosid" => $rowAgancy['loginId'],
        "appid" => $_POST['iApplicationid'],
        "status" => $_POST['fos_completed'],
