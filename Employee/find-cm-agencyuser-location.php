...
$agencymanagerid = intval($_GET['agencymanagerid']);
...
<!--    <input type="checkbox" name="check_list[]" id="check_list<?php echo $i; ?>" class="md-check " value="<?php echo $row_menu['locationId']; ?> ">-->
...
while ($row_menu = mysqli_fetch_array($result)) {
    $Client = "SELECT * FROM `agencymanagerlocation`  where isDelete='0'  and  istatus='1'   and iLocationId='" . $row_menu['locationId'] . "' and  stateId='" . $row_menu['stateId'] . "'  and iagencymanagerid = '".$agencymanagerid."' ";
