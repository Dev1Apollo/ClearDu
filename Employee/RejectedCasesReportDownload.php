...
if ($_SESSION['Type'] == 'Central Manager') {
    $sql1 = "SELECT application.strEntryDate,Bank_Name,assign_as_datetime,uniqueId,Account_No,PRODUCT,App_Id,Bkt,Customer_Name,Fathers_name,Asset_Make,Branch,customer_city,State,cycle,Allocation_Date,
        ...
        applicationpaymentcollectedhistory.penal,applicationpaymentcollectedhistory.totalamt,ptp_datetime,agencyid,fos_completed_status,application.fosid,application.applicationid,fos_submit_datetime,strFosOtherRemark
        FROM `application` left join (select applicationpaymentcollectedhistory.applicationid,status,Payment_Collected_Amount,penal,totalamt
           from 
           applicationpaymentcollectedhistory where isDelete=0 and (applicationpaymentcollectedhistory.Payment_Collected_Amount>0 or applicationpaymentcollectedhistory.penal>0 or applicationpaymentcollectedhistory.totalamt>0) ) as applicationpaymentcollectedhistory on application.applicationid=applicationpaymentcollectedhistory.applicationid " . $where . " 
        and application.is_assignto_am ='1' and  application.am_accaptance='1' and application.isDelete=0 and (application.fosid > 0 or fos_completed_status in (20))";        
} else {
    $sql1 = "SELECT application.strEntryDate,Bank_Name,assign_as_datetime,uniqueId,Account_No,PRODUCT,App_Id,Bkt,Customer_Name,Fathers_name,Asset_Make,Branch,customer_city,State,cycle,Allocation_Date,
        Allocation_CODE,Bounce_Reason,Loan_amount,Loan_booking_Date,Loan_maturity_date,Due_date,Emi_amount,Total_Pos_Amount,Total_penlty,Customer_Address,Business_Address, Contact_Number,
        alternate_contact_number,Ref_1_Name,Contact_Detail,Ref_2_Name,Contact_Detail_ref2,Area_Name,fos_comment,status,AlternetMobileNo,Pincode,Payment_Collected_Date,applicationpaymentcollectedhistory.Payment_Collected_Amount,
        ...
        FROM `application` left join (select applicationpaymentcollectedhistory.applicationid,status,Payment_Collected_Amount,penal,totalamt
           from 
           applicationpaymentcollectedhistory where isDelete=0 and (applicationpaymentcollectedhistory.Payment_Collected_Amount>0 or applicationpaymentcollectedhistory.penal>0 or applicationpaymentcollectedhistory.totalamt>0) ) as applicationpaymentcollectedhistory on application.applicationid=applicationpaymentcollectedhistory.applicationid " . $where . " and customer_city_id in  (" . $strLocationID . ") 
        and application.is_assignto_am ='1' and  application.am_accaptance='1' and application.isDelete=0 and (application.fosid > 0 or fos_completed_status in (20)) and agencyid='" . $_SESSION['agencyname'] . "'";
}

$result1 = mysqli_query($dbconn, $sql1);
//$date=date('d-m-Y');

$filename = 'rejectedcases.xls';
