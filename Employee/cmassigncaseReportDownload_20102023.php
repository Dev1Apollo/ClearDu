...
while ($rows = mysqli_fetch_array($result1)) {
    ...
    . "\t" . trim($rows['Customer_Address'])
    . "\t" . trim($rows['Business_Address'])
    // . "\t" . $rows['visit_address']
    . "\t" . $rows['Contact_Number']
    . "\t" . $rows['alternate_contact_number']
    // . "\t" . $rows['Collection_Manager']
    // . "\t" . $connect->RemoveSpecialChapr($rows['State_Manager'])
    . "\t" . trim($rows['Ref_1_Name'])
    . "\t" . trim($rows['Contact_Detail'])
    . "\t" . trim($rows['Ref_2_Name'])
    . "\t" . trim($rows['Contact_Detail_ref2'])
    . "\t" . trim($rows['Area_Name'])
    . "\t" . $fos['employeename']
    . "\t" . $fos['loginId']
    . "\t" . trim(utf8_decode($rows['fos_comment']))
    . "\t" . $rows['status']
    . "\t" . trim(utf8_decode($rows['strFosOtherRemark']))
    . "\t" . $fossubmit[0]
    . "\t" . $fossubmit[1];
    ...
    while($historyData = mysqli_fetch_assoc($sql)){
        ...
        $data .= "\t" . $historyData['status']
            . "\t" . trim(utf8_decode($historyData['comment']))
            . "\t" . $fossubmit[0]
            . "\t" . $fossubmit[1];
    }
    if($numrows < 10){
