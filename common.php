...
//	 //pagination
    function createPaging($dbconn,$table, $where, $resultPerPage, $order = '') {
        ...
        //echo $query;
        //exit;
        $fullresult = $this->Gridbind($dbconn,$table, $where);
...
//*/
 ...
 function sendmultimail($detail, $giveorder,$frommail,$fromePassword, $sub = '',$attchment='',$ccmail='',$bccmail='') {
        ...
        try {
            ...
            $mail->Password = $fromePassword;
            //$mail->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
            $mail->Port = 587;

            $mail->From = $frommail;
            $mail->FromName = "Hero_Fincorp_Pickup_Allocation";
           ...
           // $mail->AddAddress($giveorder);
            ...
            $emailids = explode(',', $ccmail);
            foreach ($emailids as $key => $value) {
                $mail->AddCC($value);
            }
           
            $emailids = explode(',', $bccmail);
            foreach ($emailids as $key => $value) {
                $mail->AddBCC($value);
            }
